...
def load_image_into_numpy_array(
    ...
    # type hint
    # Union[np.dtype, str] is used to allow both numpy dtype and string dtype, that means np.unint8 or "unit8" is all okay.
    # that means, dtype is optinal, and if provided, it should be either a numpy dtype or a string representing the dtype.
    # If dtype is not provided, it will default to None, which means the image will be loaded with its original dtype.
) -> np.ndarray:
...
class SeenUnseenSemSegEvaluator(DatasetEvaluator):
    """
    Evaluate semantic segmentation metrics for seen or unseen classes. (25.07.08)
    """
    ...
    def __init__(
        ...
        output_dir=None, 
        *,
        sem_seg_loading_fn=load_image_into_numpy_array, #corverts PIL Image of sem_segmentation GT to numpy array 
        num_classes=None,
        ignore_label=None,
    ):
        """
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            output = output["sem_seg"].argmax(dim=0).to(self._cpu_device) # shape : (C, H, W) --> (H, W)
    ...
    def evaluate(self):
        ...
        * Pixel Accuracy (pACC)
        ...
        seen_mIoU = compute_group_iou(iou, iou_valid, ade_common_ids)
        unseen_mIoU = compute_group_iou(iou, iou_valid, ade_only_ids)
        res["seen_mIoU"] = seen_mIoU
        res["unseen_mIoU"] = unseen_mIoU
